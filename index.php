...
<form action="?m=stats" method="post" name="filter">
	<table class="tbl" cellspacing="0" cellpadding="4" border="0"
		...
		<tbody>
			...
			?>
				...
				while ( $row = db_fetch_array ( $exec ) ) {
					if ($comp != $row ['project_id']) {
						$pBuffer .= ('<option value="' . $AppUI->___ ( $row ['project_id'] ) . '" style="font-weight:bold;"' . (($project . '' == $AppUI->___ ( $row ['project_id'] )) ? 'selected="selected"' : '') . '>' . $AppUI->___ ( $row ['project_name'] ) . '</option>' . "\n");
						$comp = $row ['project_id'];
					}
				}
				$pBuffer .= '</select>';
			...
			<tr><td align="center" colspan="24"><input type="submit" class="button"
					value="Filtrar" /></td></tr>
		</tbody>
	</table>
...
<?php } }
		...
		<tr class="parent" data-level="0" bgcolor="#DcDcDc">
		...
		<?php
			$select2 = "SELECT * FROM `dotp_tasks` WHERE `task_project` = '".$row['project_id']."' AND `task_parent` = `task_id`";
		...
		<tr class="parent" data-level="<?php echo $count; ?>">
			<td colspan="1"><?php for ($i=0;$i<$count;$i++){ ?><img
				src="modules/stats/toggle-expand-dark.png" height="10" weight="10">&nbsp;<?php } if($sub==1){ ?><b><?php } echo $row2['task_name']; if($sub==1){ ?></b><?php } ?></td>
		...
		<?php 
			$count++;
			loop($row['project_id'], $row2['task_id'], $row2['task_parent'], $count);
		} } ?>
	...
	<center>
		<table
			...
			<tr>
				<td colspan="2"><b>* Tarefas em negrito possuem subtarefas *</b></td>
			</tr>
		</table>
	</center>
		...
		function loop($project, $id, $parent, $count) {
			
			$select = "SELECT * FROM `dotp_tasks` WHERE `task_project` = '".$project."' AND `task_parent` = '".$id."' AND `task_parent` != `task_id`";
			$sql = db_exec($select);
			while ($row2 = db_fetch_array($sql)) {
		
				if ($row2['task_parent'] != $parent) $count++;
				...
				$select_check2 = "SELECT * FROM `dotp_tasks` WHERE `task_parent` = '".$row2['task_id']."'";
				$exec_check2 = db_exec ( $select_check2 );
				if (db_num_rows($exec_check2) >= 1) $sub = 1; else $sub = 0;
				...
				<?php
				$select_check = "SELECT * FROM `dotp_tasks` WHERE `task_project` = '".$project."' AND `task_parent` = '".$id."' AND `task_parent` != `task_id`";
				$sql_check = db_exec($select_check);
				if (db_num_rows($sql_check) > 0){
						
					loop($project, $row2['task_id'], $row2['task_parent'], $count);
		
				}
				
				$parent = $row2['task_parent'];
